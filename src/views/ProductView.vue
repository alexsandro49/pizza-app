  ...
  <main :class="{ dark: configStore.darkTheme }">
    ...
    >
      <PizzaSvg
        ...
        :pizza-color-icon="pizzaColorsPick($route.params.id)"
      />
      <p class="text-2xl mt-3">
        {{ productStore.products[$route.params.id].name }}
      </p>
      <p class="text-xl mt-3">
        R${{ productStore.products[$route.params.id].price.toFixed(2) }}
      </p>
      <p class="mt-2 text-lg text-center whitespace-pre-line">
        {{ productStore.products[$route.params.id].description }}
      </p>

      <div
        id="final"
        class="grid w-full gap-4 mt-1 grid-cols-4 items-center mt-auto h-15"
      >
        <p class="text-2xl font-bold text-center text-lg px-1">
          {{ productsQuantity[$route.params.id] }}
        </p>
        <button
          ...
          @click="reduceProductInCart($route.params.id)"
        >
          -
        </button>
        <button
          class="text-2xl font-bold w-full h-full py-1 border-2 text-center border-tomato-red rounded-lg cursor-pointer text-black hover:bg-tomato-red dark:text-white"
          @click="increaseProductInCart($route.params.id)"
        >
          +
        </button>
        ...
        >
          <font-awesome-icon
            @click="addProductToCart($route.params.id)"
